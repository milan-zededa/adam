...
func (d *DeviceManager) OnboardRemove(cn string) error {
	cert, _, err := d.OnboardGet(cn) // acquires and releases RLock internally
	if err != nil {
		return err
	}
	d.m.Lock()
...
func (d *DeviceManager) DeviceRemove(u *uuid.UUID) error {
	cert, _, _, err := d.DeviceGet(u) // acquires and releases RLock internally
	if err != nil {
		return err
	}
	d.m.Lock()
...
func (d *DeviceManager) DeviceRegister(unew uuid.UUID, cert, onboard *x509.Certificate, serial string, conf []byte) error {
	...
	u, err := d.DeviceCheckCert(cert) // acquires and releases RLock internally
	if err != nil {
		return err
	}
...
// d.m must be held by the caller for reading at least.
...
func (d *DeviceManager) GetDeviceOptions(u uuid.UUID) ([]byte, error) {
	// Use write lock since we may lazily initialise dev.Options.
	d.m.Lock()
	defer d.m.Unlock()
	dev, ok := d.devices[u]
	if !ok {
...
func (d *DeviceManager) GetGlobalOptions() ([]byte, error) {
	// Use write lock since we may lazily initialise globalOptions.
	d.m.Lock()
	defer d.m.Unlock()
